...
s2jj    = zeros(G3,1); % sampled spectrum estimator
...
SvvTvj     = Svv*Tjv';
for g3 = 1:G3
    s2jj(g3) = abs(Tjv(g3,:)*SvvTvj(:,g3));
end
% Update Gamma
for g = 1:G
    index        = groups{g};
    etha(index)  = sqrt( (1./4).^2 + (alpha*rho).*sum( s2jj(index) + pi2jj(index) ) ) - 1./4;
end
...
for iter_outer = 1:maxiter_outer
    ...
    alpha                           = ( length(idx_alpha)/2 + s_alpha )/( sum( ( s2jj(idx_alpha) + pi2jj(idx_alpha) )./( lambda(idx_alpha))) + r_alpha );
    %% rho
    f_aux                           = @(k_aux) r_rho + sum(ones(G3,1)./(1-lambda))/G3 - (s_rho - 1/2)/k_aux - trascendent_term(k_aux);
